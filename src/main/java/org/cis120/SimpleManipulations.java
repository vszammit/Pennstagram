...
public class SimpleManipulations {
    ...
    public static PixelPicture scaleColors(PixelPicture pic, double rfactor, double gfactor, double bfactor) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int r = (int)Math.round(rfactor * p.getRed());
                int g = (int)Math.round(gfactor * p.getGreen());
                int b = (int)Math.round(bfactor * p.getBlue());
                bmp[row][col] = new Pixel(r, g, b);
            }
        }
        return new PixelPicture(bmp);
        ...
        if (w1 != w2 || h1 != h2) {
            ...
            for (int col = 0; col < w1; col++) {
                for (int row = 0; row < h1; row++) {
                    ...
                    int r = weightedAverage(alpha, p.getRed(), p2.getRed());
                    int g = weightedAverage(alpha, p.getGreen(), p2.getGreen());
                    int b = weightedAverage(alpha, p.getBlue(), p2.getBlue());
                    bmp[row][col] = new Pixel(r, g, b);
                }
            }
            return new PixelPicture(bmp);

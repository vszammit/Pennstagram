...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        int avg = (int) Math.round(sum / (w * h * 3));

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int r = (int) Math.round((multiplier * (p.getRed() - avg))) + avg;
                int g = (int) Math.round((multiplier * (p.getGreen() - avg))) + avg;
                int b = (int) Math.round((multiplier * (p.getBlue() - avg))) + avg;
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int frq = 1;
                if (map.contains(p)) {
                    frq = map.getValue(p) + 1;
                }
                map.put(p, frq);
            }
        }
        Pixel[] sortList = map.getSortedPixels();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                Pixel temp = sortList[0];
                for (int i = 1; i < numColors; i++) {
                    if (p.distance(sortList[i]) < p.distance(temp)) {
                        temp = sortList[i];
                    }
                    bmp[row][col] = temp;
                }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        int h = pic.getHeight();
        Pixel[][] bmp = pic.getBitmap();
        Pixel[][] bmp1 = new Pixel[h][w];

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel newPic = avgColor(row, col, w, h, radius, bmp);
                bmp1[row][col] = newPic;
            }
        }
        return new PixelPicture(bmp1);
    }

    public static Pixel avgColor(int r, int c, int width, int height, int radius, Pixel[][] bmp) {
        int minR = r - radius;
        int maxR = r + radius;
        int minC = c - radius;
        int maxC = c + radius;
        ...
        int avgR = (int)Math.round(totalR / sum);
        int avgG = (int)Math.round(totalG / sum);
        int avgB = (int)Math.round(totalB / sum);
        Pixel newP = new Pixel(avgR, avgG, avgB);
